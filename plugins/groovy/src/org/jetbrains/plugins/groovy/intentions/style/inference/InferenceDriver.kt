...
class InferenceDriver(val method: GrMethod) {
  ...
  // To find which arguments are closures we just need to analyze any call of the processed method.
  ...
  fun collectOuterCalls(session: GroovyInferenceSession) {
    ...
    virtualMethod.parameters.zip(method.parameters).forEach { (virtualParameter, realParameter) ->
      if (virtualParameter in closureParameters.keys) {
        setUpClosuresSignature(session, closureParameters[virtualParameter]!!, ReferencesSearch.search(realParameter).findAll());
      }
